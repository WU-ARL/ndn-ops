...
sudo su - nlsr -c 'export HOME=/var/lib/ndn/nlsr/; ndnsec-cert-gen -S 201410080000 -E 201510080000 -N "UIUC Operator" -s /ndn/edu/uiuc -p /ndn/edu/uiuc/%C1.Operator/ndnops -r ~nlsr/unsigned_operator.cert > /etc/ndn/nlsr/keys/operator.cert'
sudo su - nlsr -c 'export HOME=/var/lib/ndn/nlsr/; ndnsec-key-gen -n /ndn/edu/uiuc/%C1.Router/ndnx > ~nlsr/unsigned_router.cert'
sudo su - nlsr -c 'export HOME=/var/lib/ndn/nlsr/; ndnsec-cert-gen -S 201410080000 -E 201510080000 -N "UIUC Router ndnx" -s /ndn/edu/uiuc/%C1.Operator/ndnops -p /ndn/edu/uiuc/%C1.Router/ndnx -r ~nlsr/unsigned_router.cert > /etc/ndn/nlsr/keys/router.cert'
